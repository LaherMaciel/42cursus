...
int	fdputpoint(int fd, unsigned long long nbr, char *base, unsigned long long size);
...
int	ft_fdprintaux(int fd, const char format, va_list args)
{
	...
	if (format == 'X')
		return (ft_fdputnbr(fd, va_arg(args, unsigned int), "0123456789ABCDEF", 16));
	if (format == 'x')
		return (ft_fdputnbr(fd, va_arg(args, unsigned int), "0123456789abcdef", 16));
	if (format == 'p')
