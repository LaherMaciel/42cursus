 ...
 * specified base size (`size`) and prints the corresponding character from the
 * custom base (`base`). It also handles negative numbers by printing a '-' sign
 * if the input number is less than 0. The result is printed to the standard
 * output.
 *
 * @attention This function should be reviewd and improved. And maybe merged
 * with the `putpointer()` function. Like if the `size` is bigger then 16 then
 * it handles it as a pointer and do all the pointer formatation and if not
 * works as normally
 *
...
int	ft_fdputnbr(int fd, double nbr, char *base, double size)
{
	...
	cont += write (fd, &base[((unsigned long) nbr)
